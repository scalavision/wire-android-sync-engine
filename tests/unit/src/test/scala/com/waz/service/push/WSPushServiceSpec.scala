...
import com.waz.znet.{AccessTokenProvider, JsonObjectResponse, WebSocket}
...
class WSPushServiceSpec extends ZMockSpec {
  ...
  private val httpRequest = Http.Request.create[Http.RawBody](new URL("http://www.test.com"))
  ...
  feature("WSPushService") {
    ...
    scenario("On activation should get access token and become connected. On deactivation become disconnected immediately.") {
      ...
      (webSocketFactory.openWebSocket(_: Http.Request[Http.RawBody])(_: EventContext))
